...
通过对服务器端的操作进行封装，我们可以看到这个类和客户端的类结构以及封装思路是差不多的，并且两个类的内部有些操作的重叠的：接收和发送通信数据的函数`recvMsg()`、`sendMsg()`，以及内部函数`readn()`、`writen()`。
...
private:
	...
	int readn(char* buf, int size);
	int written(const char* buf, int size);
	int m_fd; //通信的套接字
};
...
int TcpSocket::written(const char* buf, int size)
{
	int readplace = 0; //开始读取数据的坐标
	int remain = size;
	const char* space = buf;
	while (remain > 0)
	{
		if ((readplace = write(m_fd, space, remain)) > 0)
		{
			space += readplace;
			remain -= readplace;
		}
...
int TcpSocket::SendMsg(string msg)
{
	...
	int sizeflag = htonl(msg.size());
	memcpy(buf, &sizeflag, 4);
	memcpy(buf + 4, msg.data(), msg.size());
	int ret = written(buf, msg.size() + 4);
	delete[] buf;
	return ret;
}
...
int TcpSocket::readn(char* buf, int size)
{
	...
	while (remain > 0)
	{
		if ((readplace = read(m_fd, space, remain)) > 0)
		{
			space += readplace;   //space指针后移,以下次读取
			remain -= readplace;  //读取大小跟随减小
		}
		else if (readplace == -1)
		{
			return -1;
...
{
	int len = 0;
	readn((char*)&len, 4); //从描述符里面读取前四位,将大端数据放入len
	len = ntohl(len);
	cout << "the size of msg : " << len << endl;
...
public:
	...
	~TcpServer();
	int SetListen(unsigned short port);
	TcpSocket* AcceptConnect(sockaddr_in* addr = nullptr);
...
int TcpServer::SetListen(unsigned short port)
{
	...
	int ret = bind(l_fd, (sockaddr*)&addr, sizeof(addr));
	if (ret == -1)
	{
		perror("bind");
		return -1;
	...
	cout << "set listen sucessfully" << endl;
	return ret;
}
...
TcpSocket* TcpServer::AcceptConnect(sockaddr_in* addr)
{
	...
	int m_fd = accept(l_fd, (sockaddr*)addr, &addrlen);
	sleep(1);
	if (m_fd == -1)
	{
		perror("accept");
...
{
	SocketPackage* pkg = static_cast<SocketPackage*>(args);
	char ip[32];
	cout << "client ip : " << inet_ntop(AF_INET, &pkg->addr.sin_addr.s_addr, ip, sizeof(ip)) << endl;
	cout << "client port : " << ntohs(pkg->addr.sin_port) << endl;

	//通信
	while (1)
	{
		cout << "client say :: ";
		string msg = pkg->msgtcp->ReadMsg();
		if (!msg.empty())
			cout << msg << endl;
		else
			break;
...
int main()
{
	...
	while (1)
	{
		SocketPackage* pkg = new SocketPackage;
		TcpSocket* msgret = server.AcceptConnect(&pkg->addr);
		if (msgret == nullptr)
		{
			cout << "new fail again" << endl;
			continue;
		}
		//创建线程
		pkg->lsntcp = &server;
		pkg->msgtcp = msgret;
		thread child(Working,pkg);
		child.detach();
	}
	return 0;
}
...
{
	TcpSocket client;
	int ret = client.ConnectToHost("192.168.200.129", 10000);
	if (ret == -1)
	{
		return -1;
	}
	...
	//开始发数据
	int tmpfd = open("english.txt", O_RDWR);
	char buf[100];
	memset(buf, 0, sizeof(buf));
	int len = 0;
	while ((len = read(tmpfd, buf, sizeof(buf))) > 0)
	{
		client.SendMsg(string(buf,len));
...
通过这种方式可以`同时监测多个文件描述符并且这个过程是阻塞的，一旦检测到有文件描述符就绪（ 可以读数据或者可以写数据）程序的阻塞就会被解除，之后就可以基于这些（一个或多个）就绪的文件描述符进行通信了。`
...
## 7.2  select
### 7.2.1 函数原型
使用select这种IO多路转接方式需要调用一个同名函数`select`
这个函数是跨平台的，`Linux、Mac、Windows`都是支持的。通过调用这个函数可以委托内核帮助我们检测若干个文件描述符的状态，`其实就是检测这些文件描述符对应的读写缓冲区的状态`：

- 读缓冲区：检测里边有没有数据，如果有数据该缓冲区对应的文件描述符就绪
- 写缓冲区：检测写缓冲区是否可以写(有没有容量)，如果有容量可以写，缓冲区对应的文件描述符就绪
- 读写异常：检测读写缓冲区是否有异常，如果有该缓冲区对应的文件描述符就绪

委托检测的文件描述符被遍历检测完毕之后，已就绪的这些满足条件的文件描述符会通过`select()`的参数分3个集合传出,我们得到这几个集合之后就可以分情况依次处理了。
...
> `rdset`用于保存要检测的原始数据，这个变量不能作为参数传递给select函数，因为在函数内部这个变量中的值会被内核修改，函数调用完毕返回之后，里边就不是原始数据了，大部分情况下是值为1的标志位变少了，不可能每一轮检测，所有的文件描述符都是就行的状态。因此需要通过`rdtemp``变量将原始数据传递给内核，select()
...
要解决阻塞问题，就将套接字默认的阻塞行为修改为非阻塞，需使用`fcntl()`函数进行处理：
...
在UDP通信过程中，`哪一端是接收数据的角色，那么这个接收端就必须绑定一个固定的端口`，如果某一端不需要接收数据，这个绑定操作就可省略不写，通信的套接字会自动绑定一个随机端口。
...
UDP的广播和日常的广播是一样的，都是一种快速传播消息的方式，因此`广播的开销很小`，发送端使用一个广播地址，就可以将数据发送到多个接收数据的终端上，如果不使用广播，就需要进行多次发送才能将数据分别发送到不同的主机上。
